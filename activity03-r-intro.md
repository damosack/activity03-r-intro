...
``` r
tsmatrix<- matrix(data=top_secret4, ncol = 5)
evens <- seq(2, 502, 2)
step7 <- ts[,1] - evens
```

    ## Error in ts[, 1]: object of type 'closure' is not subsettable

``` r
step8 <- ts[18:24, 3] - 100
```

    ## Error in ts[18:24, 3]: object of type 'closure' is not subsettable

``` r
step9 <- ts[, 4:5] *2
```

    ## Error in ts[, 4:5]: object of type 'closure' is not subsettable

``` r
step10 <- c(step9)
```

    ## Error in eval(expr, envir, enclos): object 'step9' not found
