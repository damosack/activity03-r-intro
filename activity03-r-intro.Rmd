...
**Response**:
I figured it was probably to do with punctuation judging from the nature of the vectors, so I just looked for mismatched quotations, and missing commas. The comma was an easy fix, where as the quotations breaking the punctuation vector had to be subbed for single quotes to get R to understand the code.
...
**Response**: I'm not exactly sure what cbind does, but to make a vector the function c() is used, so the function needed to be corrected.
...
**Response**: data.frame was missing its period, which was an easy fix: insert one. Symbol 'object name not found' made me think for a minute, but since we are giving R a name instead of creating an object, it made me realize that Symbol should be in quotes.
...
**Response**: Length() returned a length of 1, which was clearly not right, so I simply called my_symbols and saw that there were 64 entries in the vector.
...
**Response**: I noticed that we are referring to a variable in the vector, and in my experience that's done with a dollar sign. I replaced the symbol which separated "my_symbol" from "num" with a dollar sign, whatever it was before.
